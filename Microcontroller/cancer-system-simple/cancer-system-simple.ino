...
// ===== RR buffer untuk HRV =====
#define MAX_NN_INTERVALS 512
float rrIntervals[MAX_NN_INTERVALS];
int   rrCount = 0;
...
// ===== FFT untuk HF =====
float vReal[MAX_NN_INTERVALS];
float vImag[MAX_NN_INTERVALS];
...
float calculateHF(const float* rr, int count) {
  ...
  for (int i = count; i < MAX_NN_INTERVALS; i++) {
    vReal[i] = 0.0f;
    vImag[i] = 0.0f;
  }
